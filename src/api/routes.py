...
from api.models import db, User, Evento, Categoria, Locacion, Funcion, Orden, Compra, Ticket
...
def new_event():
    try:
        ...
        evento = Evento(name, categoria_id, locacion_id,
                        descripcion, sinopsis, precio, duracion, imagen)
...
def new_function():
    try:
        ...
        funcion = Funcion(evento_id, datetime.strptime(
...
@api.route('/locacion', methods=['GET'])
def get_user_locacion():
    try:
        response_body = ([{
            '_id': locacion.id,
            '_name': locacion.name

        } for orden in Orde.query.all()
        ])
        return jsonify(response_body), 200
    except Exception as e:
        print("ERROR! "f'{e}')
...
def get_user_orden():
    try:
       ...
       # user_id= User.query.filter_by(id='1').first()
        ...
        join_query = db.session.query(Compra, Orden, Ticket, User, Funcion, Evento, Locacion)\
            .join(Orden, Orden.id == Compra.orden_id)\
            .join(User, User.id == Orden.usuario_id)\
            .join(Ticket, Ticket.id == Compra.ticket_id)\
            .join(Funcion, Funcion.id == Orden.funcion_id)\
            .join(Evento, Evento.id == Funcion.evento_id)\
            .join(Locacion, Locacion.id == Evento.locacion_id)
        print(join_query)
        for row in join_query.filter_by(id=user_id):
            print(row, "ROW=====")
            if row:
                datosFuncion = row.Funcion
                datosUsuario = row.User
                datosEvento = row.Evento
            response_body = {
            'Fecha': datosFuncion.fecha,
            'Hora': datosFuncion.hora,
            'nombre': datosUsuario.name,
            'apellido': datosUsuario.lastname,
            'nombre_evento': datosEvento.name,
            'precio': datosEvento.precio,
            'duracion': datosEvento.duracion
             }
            return jsonify(response_body), 200
