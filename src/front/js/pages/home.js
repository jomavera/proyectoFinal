...
export const Home = () => {
  ...
  const filtrar = (texto) => {
    const filtrado = filtrando.filter((elemento) => {
      ...
      if (
        elemento.titulo.toString().toLowerCase() == texto.toLocaleLowerCase() &&
        selectedOption.value == 3
      ) {
        return elemento;
      }
      if (
        elemento.comuna.toString().toLowerCase() == texto.toLocaleLowerCase() &&
        selectedOption.value == 4
      ) {
        return elemento;
      }
      if (
        elemento.nombre_locacion.toString().toLowerCase() ==
