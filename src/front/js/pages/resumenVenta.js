 ...
 console.log(selectedOption.value)
	const filtrar = (texto) => {
		...
		const filtrado = filtrando.filter((elemento) => {
			...
			if (elemento.nombre_evento.toLocaleLowerCase() == texto.toLowerCase() && selectedOption.value == 1) {
				return elemento
			}
			if (elemento.name.toLocaleLowerCase() == texto.toLowerCase() && selectedOption.value == 2) {
				return elemento
			}
		})
	...
	ventas.map((item) => {
		...
		data.push({
			"fecha": fechacompleta, "hora": item.hora, "duracion": item.duracion,
			"ticket_id": item.ticket_id, "precio": item.precio, "name": item.name,
			"nombre_evento": item.nombre_evento, "ubicacion": item.ubicacion, "id": item.id
		})
